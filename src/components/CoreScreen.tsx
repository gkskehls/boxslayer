...
const CoreScreen: React.FC = () => {
  const { player, playerCores, equippedCore, acquireCore, equipCore, unequipCore, upgradeCore, removeCore } = useGameStore();
  ...
  const tempCores: Core[] = [
    { id: 'fire_1', name: '불의 코어', type: 'FIRE', level: 1 },
    { id: 'water_1', name: '물의 코어', type: 'WATER', level: 1 },
    { id: 'wind_1', name: '바람의 코어', type: 'WIND', level: 1 },
    { id: 'elec_1', name: '전기의 코어', type: 'ELECTRIC', level: 1 },
  ];
        ...
        {selectedCore && (
            <div className={`p-4 rounded-lg border ${getCoreTypeColor(selectedCore.type)}`}>
              ...
              <div className="flex gap-2 mb-4">
                <button onClick={() => { if (isEquippedSelected) unequipCore(); else equipCore(selectedCore.id); setSelectedCore(null); }}
                        className={`flex-1 py-3 rounded font-bold text-sm ${isEquippedSelected ? 'bg-red-600' : 'bg-blue-600'}`}>
                  {isEquippedSelected ? '해제' : '장착'}
                </button>
                <button onClick={() => { if (window.confirm("정말 삭제하시겠습니까?")) { if (isEquippedSelected) unequipCore(); removeCore(selectedCore.id); setSelectedCore(null); } }}
                        className="px-4 py-3 bg-neutral-700 hover:bg-neutral-600 rounded font-bold text-sm text-white">
                  삭제
                </button>
              </div>
              <div className="space-y-2">
                <div className="text-[10px] text-neutral-300 text-center">비용: +1 ({getBatchCost(1)}G) | +10 ({getBatchCost(10)}G) | +100 ({getBatchCost(100)}G)</div>
                <div className="flex gap-2">
                  {[1, 10, 100].map(amt => (
                      <button key={amt} onClick={() => upgradeCore(selectedCore.id, amt)} className="flex-1 py-3 bg-yellow-700 hover:bg-yellow-600 rounded font-bold text-sm">+{amt}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        {showAcquireModal && (
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
              <div className="bg-neutral-900 p-6 rounded-lg grid grid-cols-2 gap-4">
                {tempCores.map(c => (
                    <button key={c.id} onClick={() => { acquireCore({ ...c, id: Math.random().toString() }); setShowAcquireModal(false); }}
                            className={`p-4 border ${getCoreTypeColor(c.type)}`}>{c.name}</button>
                ))}

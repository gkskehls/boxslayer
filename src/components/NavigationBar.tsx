...
const NavigationBar: React.FC<NavigationBarProps> = ({ onNavigate, currentScreen }) => {
  ...
  return (
      <div className="fixed bottom-0 left-0 right-0 bg-neutral-900 border-t-2 border-neutral-800 flex justify-evenly p-2 z-50 gap-1 overflow-x-auto">
        ...
        {navItems.map((item) => (
            <button
                ...
                // 활성화된 메뉴는 노란색 배경에 검은 글씨로 반전되어 직관성을 높입니다.
            ...
            ${currentScreen === item.screen
                    ? 'text-neutral-900 bg-yellow-400 border-yellow-400'
                    : 'text-neutral-400 border-transparent hover:text-white hover:border-neutral-500 hover:bg-neutral-800'}`}
            >

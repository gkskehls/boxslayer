...
- **데이터 경량화 (Core System):**
  - 코어 데이터 객체는 `id`, `type`, `level` 등 최소한의 식별자와 상태값만 포함합니다.
  - 파생 데이터(가격, 능력치 설명, 효과)는 컴포넌트 내부의 계산 함수(`getCoreStats`, `getUpgradeCost`)를 통해 동적으로 산출합니다.
- **상태 무결성 (Integrity):**
  - 상태 변경(강화, 구매 등)은 전역 스토어 내에서 반드시 비용 검증(Guard Clause)을 거쳐야 하며, 골드가 마이너스가 되는 것을 방지합니다.
- **반응형 상태 동기화:**
  - 강화 등 상태 변경 시, UI에 즉각적으로 반영되도록 상태 업데이트 직후 최신 스토어 값을 조회하여 동기화합니다.